...
- 图文卖点组：模板预留功能点区域，规划阶段由 AI 预填带事实引用的“视觉概念 + 短标题 + 说明”，用户确认后再生产。新模板默认把图形、标题和说明连同每个单元格的位置、大小约束一起交给主图模型，使完整卖点模块作为场景材质、浮雕、光影装置或精致信息标识自然生成在底图中，不再后贴图标或重复叠字；主标题和正文仍由平台确定性排版。旧项目的“无字视觉融合 + 后排文字”和独立透明图标层继续兼容，并保留 QA 审计与分层导出；
...
6. 点击 AI 内容规划；按页或按字段采用建议，再人工修改标题、正文、视觉目标、卖点视觉概念、卖点标题和说明。默认“整组随主图生成”会在下一次生图时按模板位置和大小把图形与卖点文字一次生成到底图中；生成后如需改卖点内容或位置，需修改规划或模板并重新生成本页。重新规划不会直接覆盖当前版本。
7. 保存草稿或确认规划；确认后项目进入“已策划”状态。
8. 选择已发布配方和本次生成质量开始生产，查看质检问题和位置标注，对每页选择最终候选。
9. 在任一候选图上点击“继续修改此图”，输入背景、场景、光影、材质或角度等自然语言要求；系统编辑该候选底图，并按模板保留或重新生成完整卖点模块，再重新排版主标题/正文并仅质检本页，原候选保留。主标题和正文修改仍可使用“调整文字排版”，不会调用生图模型。
