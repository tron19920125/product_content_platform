...
7. 在任一候选图上点击“调整文字排版”，修改字体风格、字号、颜色、对齐、位置和间距后应用；系统只生成新的透明文字层并重新质检，不会再次调用生图模型。
8. 有两张及以上候选图时，可在“长图拼接与导出”中勾选图片、调整顺序、方向、对齐、间距和背景色，生成原始分辨率 PNG 长图。
9. 全部页面确认后导出 ZIP；对有效项目点击“沉淀为配方”，由管理员测试并发布。
10. 批量任务可选择快速录入或 CSV/XLSX 导入，按整批配方生产并进入各 SKU 审核。

## 最小黄金演示

1. 点击“新建项目 → 创建黄金演示项目”。系统会直接创建并确认一页 `2048x2048` 生活场景规划，默认选择“高端生活场景演示配方”和 `High` 质量。
2. 上传至少一张商品外观参考图；需要新角度或保留局部细节时，可继续上传其他外观图和细节图并点击“上传并绑定”。示例不内置任何第三方商品素材，避免把未授权图片带入仓库。
3. 点击“开始生产”。平台会把全部商品/细节参考图发送给图片模型，由模型直接生成商品、角度、环境效果与生活场景；营销文字不进入生图模型，而是由确定性文字层统一排版。
4. 单图通过后确认候选并导出；推荐模式的交付 ZIP 包含 `base.png`（模型生成的商品与场景）、`text_layer.png`、`final.png` 和 `qa.json`。旧的 `layered_product` 兼容配方才会额外导出 `background.png` 与 `product_layer.png`。
