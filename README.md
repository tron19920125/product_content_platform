...
2. 上传一张背景干净、商品完整的正面或 45° 商品参考图，并点击“上传并绑定”。示例不内置任何第三方商品素材，避免把未授权图片带入仓库。
3. 点击“开始生产”。图片模型只生成高端住宅洗护场景；系统随后合成原样商品层和确定性文字层，再执行 OCR、参考一致性和 LLM 视觉审查。
4. 单图通过后确认候选并导出；交付 ZIP 应包含 `background.png`、`product_layer.png`、`base.png`、`text_layer.png`、`final.png` 和 `qa.json`。
