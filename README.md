...
- 图文卖点组：模板预留功能点区域，规划阶段生成带事实引用的“图标 + 短标题 + 说明”，GPT Image 2 生成无字图标；部署支持透明背景时直接输出透明 PNG，不支持时由平台确定性去除纯白背景。图标与可编辑文字确定性合成，并支持组/子项编辑、单图标重生或替换、QA 审计及分层导出；
- 平台内置审查计划、文字检查、视觉证据、候选评分排序和修复 Prompt；
- 排版问题自动调整，参考一致性问题自动重生成一次并保留修复前记录；
- 候选审核、P0/P1 覆盖原因、单页重生成，以及任意文本框的字体、字号、粗体/斜体/下划线/删除线、颜色、对齐、位置和行距修改后仅重排文字层；首次生图即创建并应用 AI 文字文档 v1，质检页可基于同一文档按自然语言生成可见的新排版版本；右侧二级字体库提供 16 款经 OFL 再分发审计的中英文字体并实时预览；
...
2. 查看商品事实，上传商品、细节、品牌或场景参考素材。
   内容规划预览会在商品区域显示真实商品参考图；上传“商品外观”素材后自动替换默认示例图，标题和正文会按模板框同比例缩放并完整展示。
...
- `docs/feature-highlights-workflow.md`（图文卖点组、透明图标、页面图层编辑、质检与分层交付）
