...
- 图文卖点组：模板预留功能点区域，规划阶段由 AI 预填带事实引用的“视觉概念 + 短标题 + 说明”，用户确认后再生产。卖点图形和卖点文案连同每个单元格的精确位置约束一起交给主图模型，作为高对比、强轮廓、缩略图可辨的电商信息模块原生生成在底图中；不再生成、上传或后贴独立透明图标。主标题和正文仍由平台确定性排版，QA 会对每个卖点图形区域执行显著性测量和多模态审查；
...
- `docs/feature-highlights-workflow.md`（图文卖点组、主图原生视觉、页面图层编辑、质检与交付）
