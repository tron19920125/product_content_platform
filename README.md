...
6. 生成内容规划，修改标题、正文、视觉目标和兼容模板。
7. 保存草稿或确认规划；确认后项目进入“已策划”状态。
8. 选择已发布配方和本次生成质量开始生产，查看质检问题和位置标注，对每页选择最终候选。
9. 在任一候选图上点击“调整文字排版”，修改字体风格、字号、颜色、对齐、位置和间距后应用；系统只生成新的透明文字层并重新质检，不会再次调用生图模型。
...
首次启动会从 `examples/showcases/` 幂等创建三个已完成、可直接打开和导出的示例项目，不依赖数据库文件，也不需要调用 Azure：
