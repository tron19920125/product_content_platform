    ...
    <section className="template-editor-panel">
      ...
      <div className="template-editor-workspace">
        <aside className="editor-layers"><header><strong>图层 / 区域</strong><Icon name="layers"/></header>
          ...
          <button className="editor-add-text feature" disabled={draft.feature_slots.length >= 3} onClick={() => { const id = `feature-${crypto.randomUUID()}`; const slot: TemplateDefinition["feature_slots"][number] = { id, name: `图文卖点组 ${draft.feature_slots.length + 1}`, box: [.08, .56, .48, .88], layout: "row", columns: 3, min_items: 2, max_items: 3, icon_position: "top", visual_mode: "scene_baked", icon_scale: .28, item_gap: .025, icon_text_gap: .012, card_style: {}, title_style: { font_size: 48 }, description_style: { font_size: 28 } }; commit({ ...draft, feature_slots: [...draft.feature_slots, slot] }); setActiveKey(`feature:${id}`); }}><Icon name="plus"/><span>新增图文卖点预留区</span></button>
        ...
        <aside className="editor-inspector">
          ...
          <section><h4>位置与尺寸</h4>
            ...
            {activeKey.startsWith("feature:") && (() => {
              const id = activeKey.slice(8);
              const slot = draft.feature_slots.find((item) => item.id === id);
              if (!slot) return null;
              const update = (changes: Partial<typeof slot>) => commit({ ...draft, feature_slots: draft.feature_slots.map((item) => item.id === id ? { ...item, ...changes } : item) });
              const featureMode = slot.visual_mode ?? "scene_baked";
              return <div className="template-feature-properties">
                <label>名称<input value={slot.name} onChange={(event) => setDraft({ ...draft, feature_slots: draft.feature_slots.map((item) => item.id === id ? { ...item, name: event.target.value } : item) })} onBlur={() => commit(draft)}/></label>
                <label>生成方式<select value={featureMode} onChange={(event) => update({ visual_mode: event.target.value as typeof slot.visual_mode })}><option value="scene_baked">整组随主图生成（推荐）</option><option value="scene_integrated">无字视觉融合 + 后排文字（兼容）</option><option value="independent_icon">独立透明图标（兼容）</option></select><small>{featureMode === "scene_baked" ? "图形、标题和说明由主图模型一次生成；规划后改字或改位置都需重新生成本页。" : featureMode === "scene_integrated" ? "只生成场景融合图形，标题和说明由平台后排；适合兼容旧项目。" : "图标单独生成并在后期合成，适合需要频繁替换图标的旧页面。"}</small></label>
                <label>排布<select value={slot.layout} onChange={(event) => update({ layout: event.target.value as typeof slot.layout })}><option value="row">横向排列</option><option value="column">纵向排列</option><option value="grid">网格排列</option></select></label>
                <label>图文关系<select value={slot.icon_position} onChange={(event) => update({ icon_position: event.target.value as typeof slot.icon_position })}><option value="top">图形在上</option><option value="left">图形在左</option></select></label>
                <label>默认数量<span className="feature-count-inputs"><input type="number" min="1" max={slot.max_items} value={slot.min_items} onChange={(event) => update({ min_items: Number(event.target.value) })}/><i>至</i><input type="number" min={slot.min_items} max="6" value={slot.max_items} onChange={(event) => update({ max_items: Number(event.target.value), columns: Math.min(slot.columns, Number(event.target.value)) })}/></span></label>
                <label>列数<input type="number" min="1" max={slot.max_items} value={slot.columns} onChange={(event) => update({ columns: Number(event.target.value) })}/></label>
                <label>图形占比<input type="number" min="0.1" max="0.8" step="0.01" value={slot.icon_scale} onChange={(event) => update({ icon_scale: Number(event.target.value) })}/><small>相对于每个卖点单元格，控制模型构图提示中的图形大小。</small></label>
                <div className="feature-size-inputs"><label>标题字号<input type="number" min="12" max="512" value={Number(slot.title_style.font_size ?? 48)} onChange={(event) => update({ title_style: { ...slot.title_style, font_size: Number(event.target.value) } })}/></label><label>说明字号<input type="number" min="10" max="384" value={Number(slot.description_style.font_size ?? 28)} onChange={(event) => update({ description_style: { ...slot.description_style, font_size: Number(event.target.value) } })}/></label></div>
                <button className="danger-link" onClick={() => { commit({ ...draft, feature_slots: draft.feature_slots.filter((item) => item.id !== id) }); setActiveKey(`text:${draft.text_slots[0]?.id ?? ""}`); }}>删除图文卖点预留区</button>
              </div>;
            })()}
