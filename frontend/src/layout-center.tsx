          ...
          <section><h4>位置与尺寸</h4>
            ...
            {activeKey.startsWith("feature:") && (() => {
              ...
              return <div className="template-feature-properties">
                ...
                <label>生成方式<select value={featureMode} onChange={(event) => update({ visual_mode: event.target.value as typeof slot.visual_mode })}><option value="scene_baked">整组随主图生成（推荐）</option><option value="scene_integrated">无字视觉融合 + 后排文字（兼容）</option></select><small>{featureMode === "scene_baked" ? "高辨识度图形、标题和说明由主图模型一次生成；规划后改字或改位置都需重新生成本页。" : "只生成醒目的场景融合图形，标题和说明由平台后排；适合兼容旧项目。"}</small></label>
