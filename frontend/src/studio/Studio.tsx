...
export default function Studio() {
  ...
  const setCurrent = useCallback((value: Draft) => {
    ...
    setSelectedIds([]); setCompare(false); setCompareAssetId(null); setDetailFit(true); setAiInstruction("");
  ...
  useEffect(() => {
    void studio.catalog().then(setCatalog).catch(() => {setLoadingDraft(false); setError("创作服务未连接，请检查服务后刷新重试。示例作品仍可浏览。");});
  ...
  function navigateSection(next: "create" | "library" | "history") {
    ...
    if (next === "library") {setTrash(false); setFilter("all");}
  ...
  async function generate(pageIds?: string[], source?: Version) {
    await act(async () => {
      ...
      setSelectedVersion(null); setSelectedIds([]); setAiInstruction(""); setInputOpen(false); await refresh(saved.id);
      if (!demoMode || source) setNotice("生成任务已保存，正在等待生成服务处理。");
    });
  }
  ...
  return <div className="st-app" aria-busy={busy || loadingDraft}>
    ...
    <div className="st-main" inert={busy}><header className="st-top"><div><span className="st-breadcrumb">工作空间</span><Icon name="chevron" size={14}/><strong>{section === "create" ? active.name : section === "library" ? "素材库" : "创作记录"}</strong></div><div><span className="st-local-dot"/>本地版本<span className="st-top-separator"/><button className="st-link" onClick={() => setNotice("预置作品仅用于演示和效果参考；自定义内容需重新提交生成。")}>演示说明 <Icon name="info" size={15}/></button></div></header>
      ...
      {section === "create" ? <div className="st-workspace" inert={loadingDraft}>{inputOpen && <button className="st-input-backdrop" aria-label="收起创作设置" tabIndex={-1} onClick={() => setInputOpen(false)}/>}
        <aside className={`st-input ${inputOpen ? "open" : ""}`}><div className="st-input-heading"><button className="st-icon st-input-close" aria-label="关闭创作设置" onClick={() => setInputOpen(false)}><Icon name="close"/></button><h1>{active.name}</h1><p>{active.description}</p></div>
          <div className="st-input-scroll" onPaste={event => {const files = Array.from(event.clipboardData.files); if (files.length) {event.preventDefault(); void upload(files, "product");}}}>
            ...
            <label className="st-field">制作要求 <small>选填</small><textarea maxLength={16000} placeholder={tool === "scene_image" ? "描述想要的空间、光线和氛围…" : "描述想表达的卖点、文案或画面风格…"} rows={3} value={draft?.content.requirements ?? ""} onChange={event => change({requirements: event.target.value})}/></label>
          ...
          <div className="st-generate-footer"><div><span>{loadingDraft ? "加载草稿中…" : saveStatus}{saveStatus === "保存失败" && <button className="st-link" onClick={() => void act(async () => {await persist();})}>重试保存</button>}</span><span>{draft?.content.pages.filter(p => !p.skipped).length ?? 0} 页 × {draft?.content.candidate_count ?? 1} 张</span></div><button className="st-primary" disabled={!draft || busy || loadingDraft || !draft.content.product_asset_ids.length || !activePages.length || !aPlusReady} onClick={() => void generate()}><Icon name={demoMode ? "image" : "spark"}/>{busy ? "提交中…" : demoMode ? "回放示例" : "开始生成"}</button><small>{!aPlusReady ? "请先生成模块方案，或填写每个模块的文案与画面描述" : demoMode ? "使用预置 AI 成图，实际像素以示例为准" : "自定义任务将进入智能生成队列"}</small></div>
        </aside>
        <main className="st-results"><div className="st-results-heading"><div><h2>{results.versions.length || allJobs.length ? "创作结果" : "开始你的下一次创作"}</h2><p>{results.versions.length || allJobs.length ? "每张候选独立完成，可分别查看、选用或重试" : "上传商品图片，余下的交给你的创作工作台"}</p></div><div className="st-toolbar"><button className="st-secondary" disabled={busy || loadingDraft} onClick={() => void newCreation()}><Icon name="plus" size={17}/>新建创作</button><button className="st-secondary st-input-toggle" onClick={() => setInputOpen(!inputOpen)}>创作设置</button><button className="st-secondary" onClick={() => setHistoryOpen(!historyOpen)}><Icon name="history" size={17}/>创作记录</button></div></div>
          ...
          {!!pending.length && <div className="st-progress"><span className="st-spinner"/><div><strong>{results.operations[0]?.mode === "demo" ? "正在回放示例" : "任务已提交，等待生成"}</strong><small>已完成 {allJobs.filter(j => j.status === "done").length} 张 · 待处理 {pending.length} 项。离开页面不丢记录。</small></div><button className="st-link" onClick={() => void act(async () => {for (const operation of results.operations.filter(op => !op.stopped)) await studio.stop(operation.id); if (draft) await refresh(draft.id);})}>停止后续生成</button></div>}
          ...
          {demo && <section className="st-inspiration"><div className="st-section-line"><div><h3>灵感与示例作品</h3><p>无需生成，即可查看完整效果</p></div><button className="st-link" onClick={() => navigateSection("library")}>全部素材 <Icon name="arrow" size={16}/></button></div>{demoCards(demo.items.slice(0, 3), true)}</section>}
        </main>
        ...
        {!trash && (filter === "all" || filter === "work" || filter === "style") && demo && <section><div className="st-section-line"><h3>示例作品 <span className="st-count">{demo.items.length}</span></h3><small>已预置 · 无需现场生成</small></div>{demoCards(demo.items.filter(item => (item.title + item.tags.join(" ")).toLocaleLowerCase().includes(search.trim().toLocaleLowerCase())))}</section>}
    ...
    {confirm && <Modal busy={busy} title={confirm.title} onClose={() => setConfirm(null)}><div className="st-confirm"><p>{confirm.message}</p><div><button className="st-secondary" onClick={() => setConfirm(null)}>取消</button><button className="st-primary" onClick={() => {const action = confirm.run; setConfirm(null); void act(action);}}>确认</button></div></div></Modal>}
