...
export function TextLayoutEditor({ candidate, onComplete, onCancel }: Props) {
  ...
  return <section className="typography-editor text-layout-editor">
    <div className="tool-heading"><div><strong>页面图层工作台</strong><small>像 PPT 一样编辑文字；场景融合卖点的视觉元素保留在主图中，应用时只重做可编辑文字层。</small></div><button type="button" className="icon-button" aria-label="关闭页面图层工作台" onClick={onCancel}>×</button></div>
    ...
    <div className="text-layout-workspace">
      ...
      <aside className={`text-property-panel ${inspectorView === "fonts" ? "font-library-view" : ""}`}>
        {selectedGroup?.visual_mode === "scene_baked" ? <>
          ...
          <label className="check-control"><input type="checkbox" checked={selectedGroup.locked} disabled={selectedGroup.visual_mode === "scene_integrated"} onChange={(event) => updateFeatureGroup(selectedGroup.id, { locked: event.target.checked })}/>锁定卖点组{selectedGroup.visual_mode === "scene_integrated" ? "（与底图视觉对齐）" : ""}</label>
          <p className="feature-fact-note">{selectedGroup.visual_mode === "scene_integrated" ? "卖点视觉已按模板位置生成在主图中，因此组位置保持锁定；选择子项可修改标题、说明和文字样式。" : "在画布中拖拽或缩放整个组；选择子项可分别修改图标、标题、说明和文字样式。"}</p>
    ...
    {stageRef.current ? createPortal(<>{textDocument.feature_groups.filter((group) => group.visual_mode !== "scene_baked").map((group) => {
      const columns = group.layout === "row" ? group.items.length : group.layout === "column" ? 1 : Math.min(group.columns, group.items.length);
      return <div key={group.id} className={`editable-feature-group ${group.visual_mode === "scene_integrated" ? "scene-integrated" : ""} ${selectedGroupId === group.id ? "selected" : ""} ${group.locked ? "locked" : ""}`} style={{ left: `${group.box[0] * 100}%`, top: `${group.box[1] * 100}%`, width: `${(group.box[2] - group.box[0]) * 100}%`, height: `${(group.box[3] - group.box[1]) * 100}%`, display: group.visible ? "grid" : "none", gridTemplateColumns: `repeat(${Math.max(1, columns)}, minmax(0, 1fr))`, gap: `${Math.max(2, group.item_gap * stageSize[0])}px` }} onPointerDown={(event) => beginFeatureTransform(event, group, "move")}>
