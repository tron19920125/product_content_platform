...
class ApiTest(unittest.TestCase):
    ...
    def test_font_catalog_exposes_preview_and_license_metadata(self) -> None:
        ...
        self.assertTrue({"现代黑体", "宋体衬线", "艺术标题", "书法", "马克笔标题", "数字价格"} <= {item["category"] for item in fonts})
        ...
        self.assertTrue(all(item["license_verified"] and item["redistribution"] and item["tags"] for item in fonts))
        self.assertFalse({"zcool-kuaile", "zcool-qingke-huangyou", "zcool-xiaowei"} & {item["id"] for item in fonts})
